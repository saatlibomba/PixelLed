...
// Device ID’yi mac adresine göre al
$stmt = $pdo->prepare("SELECT id FROM devices WHERE mac = :mac");
$stmt->execute(['mac' => '08:A6:F7:10:9E:DC']);
$did = (int) $stmt->fetchColumn();
if (!$did) {
  die("Cihaz bulunamadı. Mac adresi yanlış olabilir.");
}

// Shape’leri çek
$stmt2 = $pdo->prepare("
  SELECT id, shape_name, led_indices
  FROM mirror_shapes
  WHERE device_id = :did
  ORDER BY id
");
$stmt2->execute(['did' => $did]);
$shapes = $stmt2->fetchAll(PDO::FETCH_ASSOC);

require __DIR__ . '/includes/header.php';
?>
<div class="container mt-4">
  <h2>Ayna Çerçevesi</h2>
    <style>
    #svg-container { flex:1; display:flex; justify-content:center; align-items:center; background:#fff; overflow:auto; }
    #mirrorSVG { height:90%; width:auto; max-width:100%; max-height:100%; display:block; }
    #mirrorSVG [id] { cursor:pointer; pointer-events:all; }
    #mirrorSVG .fil0 { fill:transparent !important; pointer-events:all; }
    #mirrorSVG .str0 { stroke-width:2px!important; vector-effect:non-scaling-stroke; }
  </style>
  <div id="svg-container">
    <!-- Buraya gerçek SVG’in gelecek; shape'lerin id'si 'shape1','shape2' vb. -->
    <svg id="mirrorSVG" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 91025.02 280444.28" preserveAspectRatio="xMidYMid meet">
        <defs>
  ...
  <div class="card mt-4 p-3">
    <h4>Shape Ayarları</h4>
    <div class="mb-3">
      <label for="shapeSelect">Şekil</label>
      <select id="shapeSelect" class="form-select">
        <option value="">— Tümü —</option>
        <?php foreach($shapes as $s): ?>
          <option value="<?=htmlspecialchars($s['shape_name'])?>">
            <?=htmlspecialchars($s['shape_name'])?>
          </option>
        <?php endforeach; ?>
      </select>
    </div>
    ...
    <div class="mb-3">
      <label for="animType">Animasyon</label>
      <select id="animType" class="form-select">
        <option value="static">Statik</option>
        <option value="blink">Yanıp Sön</option>
        <option value="rainbow">Gökkuşağı</option>
      </select>
    </div>
    <button id="saveMirrorBtn" class="btn btn-primary">
      Ayna Komutunu Kaydet
    </button>
  </div>
</div>

<script>
// PHP’den gelen mapping
let shapesMap = <?= json_encode(array_column($shapes, 'led_indices', 'shape_name')) ?>;
for (let k in shapesMap) {
  shapesMap[k] = JSON.parse(shapesMap[k]);
}

// Shape seçildiğinde SVG üzerinde vurgula
document.getElementById('shapeSelect').addEventListener('change', e=>{
  const sel = e.target.value;
  document.querySelectorAll('#mirrorSVG [id]').forEach(el=>{
    if (!sel || el.id === sel) {
...
// Komutu kaydet butonu
document.getElementById('saveMirrorBtn')
  .addEventListener('click', ()=>{
    ...
    let leds;
    if (shape) {
      leds = shapesMap[shape];
    } else {
      // tüm LED’ler
      leds = Object.values(shapesMap).flat();
    }
    const payload = { shape: shape || null, leds, color, anim };

    fetch('mirror_command_api.php', {
      method: 'POST',
    ...
    .then(r=>r.json())
    .then(j=>{
      if (!j.success) alert('Hata: '+j.msg);
      else      alert('Komut kaydedildi!');
    })
